    ...
    switch (comType) {
        case "form":
            if (this.ctrl.data == null) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (jsonData.status == "1") {
                            ...
                            oJsCtrl.jsonData = jsonData.results;
                            oJsCtrl.init();
                            this.ctrl.jsBodyCtrl = oJsCtrl;
                        }
						jumpUrl(null,jsonData.status,null,jsonData);
                    },
                ...
                var oJsCtrl = new clsFormCtrl();
                oJsCtrl.ctrl = this.ctrl;
                oJsCtrl.jsonData = this.ctrl.data;
                oJsCtrl.init();
                this.ctrl.jsBodyCtrl = oJsCtrl;
            }
            break;
        case "singleSelectCtrl":
            if (this.ctrl.data == null) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
						if(data.status == 1)
						{
							...
							oJsCtrl.ctrl = this.ctrl;
							oJsCtrl.jsonData = data.results;
							oJsCtrl.init();
							this.ctrl.jsCtrl = oJsCtrl;
						}
						jumpUrl(null,data.status,null,data);
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        alert(errorThrown);
                    }
                ...
                var oJsCtrl = new clsSingleSelectCtrl();
                oJsCtrl.ctrl = this.ctrl;
                oJsCtrl.jsonData = this.ctrl.data.results;
                oJsCtrl.init();
                this.ctrl.jsCtrl = oJsCtrl;
            }
            break;
                    ...
                    success: function (data) {
						if(data.status == 1)
						{
							...
							if (this.ctrl.getAttribute("parentCtrl") != null)
								oJsCtrl.parentCtrl = $(this.ctrl).parents(this.ctrl.getAttribute("parentCtrl"));
							oJsCtrl.jsonData = data.results;
							this.ctrl.jsCtrl = oJsCtrl;
							oJsCtrl.init();
						}
    ...
    if (unionSel != null) {
        ...
        if (ctrl.value != "") {
            ...
            $.ajax({
                ...
                data: JSON.stringify(this.childReqParam),//传的参数，用jsondata自己组合
                ...
                success: function (data) {
                    ...
                    if (this.jsCtrl.parentCtrl == null)
                    {
						...
						if(this.jsCtrl.isEmptyShow == 0 && data.results.length == 0)
						{
                    ...
                    for (var nI = 0; nI < data.results.length; nI++) {
                        var jsonItem = data.results[nI];
                    ...
                    if (this.jsCtrl.parentCtrl == null) {
                        ...
                        if (obj.getAttribute("unionSel") != null) {
                            $(obj).change(function (event) {
                                ...
                                if (this.getAttribute("parentCtrl") != null)
                                    oJsCtrl.parentCtrl = $(this).parents(this.getAttribute("parentCtrl"));
                        ...
                        if (obj.getAttribute("unionSel") != null) {
                            ...
                            $(this.jsCtrl.parentCtrl).find("#" + obj.id).change(function (event) {
                                ...
                                oJsCtrl.initChild(oJsCtrl.ctrl, oJsCtrl.parentCtrl, oJsCtrl.initValues, oJsCtrl.nIdx + 1);
                                this.jsCtrl = oJsCtrl;
                            });
                        }
                    }
...
function clsStandardTableCtrl() {
    ...
    this.after		= clsStandardTableCtrl$after;clsStandardTableCtrl$after;
...
function clsTreeListCtrl$getAllParent(obj) {
    ...
    var level = $(obj).parents("tr:first").attr("level");
    $(obj).parents("tr:first").prevAll().each(function () {
        var cLevel = $(this).attr("level");
        if (parseInt(level) > parseInt(cLevel)) {
...
function clsTreeListCtrl$addNode(obj) {
    ...
    if (arrChildNode.length == 0) {
        ...
        $(newNode).append("<td class='td0'></td>");
        $(newNode).append("<td class='td1'></td>");
        $(newNode).append("<td class='td2'><div class='menu" + (parseInt(obj.getAttribute("level")) + 2) + "'><span class='chWidth'><i class='shrink'></i></span><input type='checkbox'><input type='text' class='inputWidth' value=''></div></td>");

        newNode.setAttribute("level", parseInt(obj.getAttribute("level")) + 1);
        this.process(newNode);
        $(newNode).append("<td class='td3'><a href='javascript:;' id='upMove' class='moveUp moveUpGray'></a><a href='javascript:;' id='downMove' class='moveDown'></a></td>");
        if (obj.getAttribute("level") == "0")
            $(newNode).append("<td class='td4'><a href='javascript:;' id='addNode' class='proAdd'></a><a href='javascript:;' class='' id='deleteNode'></a></td>");
        else
            $(newNode).append("<td class='td4'><a href='javascript:;' style='cursor:default'></a><a href='javascript:;' class='' id='deleteNode'></a></td>");
...
function clsStandardEditTableCtrl$parse() {
    this.before();
    if (this.jsonData != null && this.templateId != null && this.jsonData.resultData != null) {
        this.cloneId = this.templateId.replace("template", "clone");
        this.clear();
        var templateRow = $(this.ctrl).find("#" + this.templateId)[0];
        if (this.noDataId != null && this.jsonData.resultData.length == 0) {
...
function clsAlertBoxCtrl$Alert(data, title,hasCancle,popType) {
    var _this = this;
    if ($("#alertBoxWin").length == 0) {
        var alertContent = this.templateAlert(data, true, title,hasCancle,popType);
        $('body:first').append(alertContent);
    }
    else {
        var alertContent = this.templateAlert(data, false, title,hasCancle,popType);
...
//模拟alert弹窗模板
function clsAlertBoxCtrl$templateAlert(data, bln, title,hasCancle,popType) {
...
function clsCountDownTimerCtrl$getTime() {
	...
	{
		case "1":
			a.innerHTML = (dayVal == "000") ? "" : dayVal+"天";
			b.innerHTML = (hVal == "00") ? "" : hVal+"小时";
			c.innerHTML = (mVal == "00") ? "" : mVal+"分";
			d.innerHTML = (sVal == "00") ? "" : sVal+"秒";
			break;
		case "2":
			$(a).show();
			$(b).show();
...
function clsPlusMinusRowCtrl$parse() {
    ...
    if (this.jsonData != null && this.templateId != null) {
        ...
        var templateRow = $(this.ctrl).find("#" + this.templateId)[0];
        if (this.jsonData != null) {
            for (var nI = 0; nI < this.jsonData.results.length; nI++) {
                var jsonItem = this.jsonData.results[nI];
                var cloneRow = templateRow.cloneNode(true);
                cloneRow.id	= this.cloneId;
                cloneRow.style.display = "";
                cloneRow.jsonData = jsonItem;
				...
				if(nI > 0)
				{
					if(nI == this.jsonData.results.length-1)
					{
				...
				else if(nI == 0 && this.jsonData.results.length == 1)
				{
					$(cloneRow).find("."+this.clsPlus).show();
...
function clsDynamicTableCtrl() {
    ...
    this.after		= clsDynamicTableCtrl$after;clsStandardTableCtrl$after;
...
//公用方法
function getAjaxResult(strPath, method, param, callbackMethod) {
    ...
    $.ajax({
        ...
        success: function (data) {
			...
			jumpUrl(null,jsonResultData.status,null,jsonResultData);
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert(errorThrown);
        }
...
function setValue4Desc(jsonItem, cloneRow) {
    for(key in jsonItem) {
		...
		{
			setValue4DescProcess(jsonItem[key]);
		}
		else
		{
			var ctrl = $(cloneRow).find("#" + key)[0];
...
function setValue4DescProcess(jsonItem)
{
